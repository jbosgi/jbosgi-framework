...
final class FallbackLoader implements LocalLoader {
    ...
    List<String> getWeavingImports() {
        return weavingImports;
    }
    ...
    private List<XPackageRequirement> getDynamicPackageRequirements(BundleRevision brev) {
        ...
        addWeavingImportRequirements(result);

        return result;
    }

    private void addWeavingImportRequirements(List<XPackageRequirement> imports) {

        if (weavingImports.isEmpty())
            return;

        // Build dummy meta builder
        OSGiMetaDataBuilder mdbuilder = OSGiMetaDataBuilder.createBuilder(hostBundle.getSymbolicName(), hostBundle.getVersion());
        for (String importSpec : weavingImports) {
            mdbuilder.addDynamicImportPackages(importSpec);
        }
        OSGiMetaData metadata = mdbuilder.getOSGiMetaData();

        // Build dummy resource
