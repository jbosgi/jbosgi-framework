...
public class BootDelegationTestCase extends OSGiTest {
    ...
    public void testBootDelegation() throws Exception {
        doTestBootDelegation(vmClass, "javax.security.auth.x500", true);
        doTestBootDelegation(vmClass, "javax.security.auth.*", true);
        doTestBootDelegation(vmClass, "javax.security.*", true);
        doTestBootDelegation(vmClass, "*", true);
        doTestBootDelegation(vmClass, null, false);
        doTestBootDelegation(vmClass, "junk.*", false);
    }

    private void doTestBootDelegation(Class<?> vmClass, String bootDelegation, boolean fromBoot) throws Exception {
        ...
        try
        {
            ...
            JavaArchive testArchive = getTestArchive();
            Bundle testBundle = sysContext.installBundle(testArchive.getName(), OSGiTestHelper.toInputStream(testArchive));
    ...
    private JavaArchive getTestArchive() {
        final JavaArchive archive = ShrinkWrap.create(JavaArchive.class, "bootdelegation-bundle");
